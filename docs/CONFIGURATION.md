...
| `CLAUDEGATE_FIRST_EVENT_TIMEOUT_S` | `90` | How long a turn may produce *nothing* before it is failed. Separate from the request timeout on purpose: a CLI that connects and then goes silent is nearly always bad credentials, and saying "timed out waiting for the model" sends people to the wrong place. |
